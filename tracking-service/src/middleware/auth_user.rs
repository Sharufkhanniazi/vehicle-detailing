...
use crate::{state::AppState, utils::errors::{AppError, Result}};
use uuid::Uuid;
    ...
    AppState: FromRef<S>,
{
    ...
    async fn from_request_parts(
        ...
        let token = TypedHeader::<Authorization<Bearer>>::from_request_parts(parts, state)
            ...
            .map_err(|_| AppError::InternalServerError("No Token Found".into()))?;

        let app_state = AppState::from_ref(state);
        
        let claims: Claims = validate_token(&token, app_state.jwt_secret.as_ref())
            .map_err(|_| AppError::InternalServerError("Invalid token".into()))?;
